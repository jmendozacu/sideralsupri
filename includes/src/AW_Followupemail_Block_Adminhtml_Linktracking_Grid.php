...
class AW_Followupemail_Block_Adminhtml_Linktracking_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    {
        return $this->getLayout()->createBlock('followupemail/adminhtml_linktracking_queryselector')->toHtml()
            . parent::_toHtml()
        ;
    }
...
// adding order-related fields
        if (AW_Followupemail_Model_Source_Linktracking_Types::LINKTRACKING_TYPE_LINK_CART_ORDER == $this->_queryType) {
            ...
            $mageVersionCode = AW_Followupemail_Helper_Data::getMagentoVersionCode();
            if ($mageVersionCode == AW_Followupemail_Helper_Data::MAGENTO_VERSION_CE_1_3)
                $this->addColumn(
                    'order_is_active',
                    array(
...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = service
    ...
    protected function _filterConditionCurrency($collection, $column)
    {
        ...
        if (isset($value['from']) && isset($value['to'])) {
            $collection->getSelect()->where(
                $this->getRealFieldName($column->getIndex()) . ' BETWEEN \'' .
                addslashes($value['from']) . '\' AND \'' . addslashes($value['to']) . '\''
            );
        } elseif (isset($value['from'])) {
            $collection->getSelect()->where(
                $this->getRealFieldName($column->getIndex()) . '>\'' . addslashes($value['from']) . '\''
            );
        } elseif (isset($value['to'])) {
            $collection->getSelect()->where(
                $this->getRealFieldName($column->getIndex()) . '<\'' . addslashes($value['to']) . '\''
            );
        }
    }
    ...
    protected function _filterConditionDate($collection, $column)
    {
        ...
        if (isset($value['from']) && isset($value['to'])) {
            $mysqlDateFormat = AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT;
            $collection->getSelect()->where(
                $this->getRealFieldName($column->getIndex()) . ' BETWEEN \'' .
                date($mysqlDateFormat, $value['from']->get(Zend_Date::TIMESTAMP))
                . '\' AND \'' .
                date($mysqlDateFormat, $value['to']->get(Zend_Date::TIMESTAMP))
                . '\''
            );
        } elseif (isset($value['from'])) {
            $collection->getSelect()->where(
                $this->getRealFieldName($column->getIndex()) . '>\''
                . date(
                    AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT,
                    $value['from']->get(Zend_Date::TIMESTAMP)
                ) . '\''
            );
        } elseif (isset($value['to'])) {
            $collection->getSelect()->where(
                $this->getRealFieldName($column->getIndex()) . '<\''
                . date(
                    AW_Followupemail_Model_Mysql4_Queue::MYSQL_DATETIME_FORMAT, $value['to']->get(Zend_Date::TIMESTAMP)
                ) . '\''
            );
        }
    }
    ...
    protected function _filterCondition($collection, $column)
    {
        ...
        $collection->getSelect()->where(
            $this->getRealFieldName($column->getIndex()) . '="' . mysql_escape_string($value) . '"'
        );
    }
