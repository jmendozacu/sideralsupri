...
class AW_Followupemail_Model_Events
{
    ...
    */
    ...
    public function newsletterSubscribe($event)
    {
        ...
        $queue = Mage::getResourceModel('followupemail/queue');
        $statusKey = 'subscriber_status';
        $emailKey = 'subscriber_email';
        ...
        if (count($ruleIds)) {
            $params = array(
                ...
                'subscriber_status' => $subscriber->getData($statusKey)
            );
            ...
            foreach ($ruleIds as $ruleId) {
                if ($subscriber->getData($statusKey) === 1) {//process queue if subscription status is active
                    Mage::getModel('followupemail/rule')->load($ruleId)->process($params, $objects);
                }
            }
        }
