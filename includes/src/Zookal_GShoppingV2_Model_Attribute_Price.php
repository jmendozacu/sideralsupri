 ...
 */
class Zookal_GShoppingV2_Model_Attribute_Price
    extends Zookal_GShoppingV2_Model_Attribute_Default
{
    ...
    {
        $sp = $this->_dispatch('gshoppingv2_attribute_price', $product, $shoppingProduct);
        if ($sp !== null) {
            return $sp;
        }
        ...
        if (!is_null($salePriceMapValue) && floatval($salePriceMapValue) > .0001) {
            ...
            $finalPrice = Mage::helper('gshoppingv2/price')->getCatalogPrice($product, $store, $inclTax);
        }
        ...
        if (!is_null($priceMapValue) && floatval($priceMapValue) > .0001) {
            ...
            $price = Mage::helper('gshoppingv2/price')->getCatalogRegularPrice($product, $store);
        } else {
            $inclTax = ($priceDisplayType != Mage_Tax_Model_Config::DISPLAY_TYPE_EXCLUDING_TAX);
            $price   = Mage::helper('gshoppingv2/price')->getCatalogPrice($product, $store, $inclTax);
        }
        if ($product->getTypeId() != Mage_Catalog_Model_Product_Type::TYPE_BUNDLE) {
