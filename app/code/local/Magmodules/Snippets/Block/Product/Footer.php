...
class Magmodules_Snippets_Block_Product_Footer extends Mage_Core_Block_Template {
			...
			if($type == 'footer') {
				if($_snippets = $this->getProductSnippets()) {
					...
					if(is_object(Mage::registry('current_product'))) {
						$this->addData(array(
							'cache_lifetime'    => 7200,
							'cache_tags'        => array(Mage_Catalog_Model_Product::CACHE_TAG, Magmodules_Snippets_Model_Snippets::CACHE_TAG),
							'cache_key'         => $storeId . 'snippets-footer-p-' . Mage::registry('current_product')->getId(),        
						));
					}
