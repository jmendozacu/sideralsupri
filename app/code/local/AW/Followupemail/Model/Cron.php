    ...
    protected function _checkOrderStatusHistory()
    {
        ...
        while ($currentPage <= $_pages) {
            ...
            foreach ($statusHistoryCollection->getItems() as $historyItem) {
                $ruleTypeOrderStatusPrefix = AW_Followupemail_Model_Source_Rule_Types::RULE_TYPE_ORDER_STATUS_PREFIX;
                $eventName = $ruleTypeOrderStatusPrefix . $historyItem->getStatus();

                $order->reset()->load($historyItem->getParentId());
                if (
                    ($eventName == $ruleTypeOrderStatusPrefix . 'pending') ||
                    ($eventName == $ruleTypeOrderStatusPrefix . 'pending_payment')
                ) {
