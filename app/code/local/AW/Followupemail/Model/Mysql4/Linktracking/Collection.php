...
class AW_Followupemail_Model_Mysql4_Linktracking_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    {
        $columns = array_merge(self::$_fields, self::$_cartFields, self::$_orderFields);
    ...
    public function getLinktrackingCollection($queryType, $fields = false)
    {
        if (!$fields) {
            ...
            $fields = array_merge(self::$_fields, is_array($fields) ? $fields : array($fields));
        }
        ...
        switch ($queryType) {
            ...
            case AW_Followupemail_Model_Source_Linktracking_Types::LINKTRACKING_TYPE_LINK_CART_ORDER :
                ...
                if (Mage::helper('followupemail')->checkVersion('1.4')) {
                    unset(self::$_orderFields['order_entity_type_id']);
                    unset(self::$_orderFields['order_attribute_set_id']);
                    unset(self::$_orderFields['order_parent_id']);
                    unset(self::$_orderFields['order_is_active']);
                }
                $this->getSelect()
                    ->columns(self::$_cartFields)
                    ->columns(self::$_orderFields)
                    ->joinInner(
                        array('q' => $this->getTable('sales/quote')),
                        'q.entity_id=e.object_id', ''
                    )
